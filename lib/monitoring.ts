...
export function recordMetric(metric: PerformanceMetrics) {
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log(`[PERF] ${metric.method} ${metric.endpoint} - ${metric.statusCode} - ${metric.duration}ms`)
  }
}
...
export function getMetricsSummary() {
  const now = Date.now()
  const lastHour = metricsStore.filter(m => now - m.timestamp.getTime() < 3600000)
  
  const summary = {
    total: metricsStore.length,
    lastHour: lastHour.length,
    averageDuration: lastHour.reduce((sum, m) => sum + m.duration, 0) / lastHour.length || 0,
    errorRate: lastHour.filter(m => m.statusCode >= 400).length / lastHour.length || 0,
