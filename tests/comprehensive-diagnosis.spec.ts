...
test.describe('Comprehensive Portfolio Diagnosis', () => {
  ...
  test('Chat Interface Diagnosis - Port 3001', async ({ page }) => {
    ...
    try {
      ...
      console.log(
        '✗ Chat bubble not found, checking for alternative selectors...'
      )
    ...
    try {
      ...
      await page.screenshot({
        path: 'debug-chat-open-failure.png',
        fullPage: true,
      })
    }
    ...
    try {
      ...
      await page.screenshot({
        path: 'debug-message-send-failure.png',
        fullPage: true,
      })
    }
    ...
    try {
      ...
      if (aiMessageCount > 1) {
        ...
        if (!content || content.trim() === '') {
          console.log(
            '⚠️  AI response is empty - this indicates the chat API issue'
          )
        } else {
    ...
    const responses = await Promise.all([
      page
        .waitForResponse(response => response.url().includes('/api/chat'), {
          timeout: 5000,
        })
        .catch(() => null),
    ])
    ...
    await page.screenshot({
      path: 'debug-chat-final-state.png',
      fullPage: true,
    })
  ...
  test('Image Loading Diagnosis', async ({ page }) => {
    ...
    const workImages = page.locator(
      'img[alt*="E-Commerce"], img[alt*="Task Management"], img[alt*="Weather"], img[alt*="Social Media"], img[alt*="AI-Powered"], img[alt*="Fitness"]'
    )
    ...
    for (let i = 0; i < workImageCount; i++) {
      ...
      console.log(
        `Work Image ${i + 1}: Alt="${alt}", Src="${src}", Visible=${isVisible}`
      )
      ...
      if (naturalWidth === 0 || naturalHeight === 0) {
        console.log(
          `⚠️  Image ${i + 1} failed to load (dimensions: ${naturalWidth}x${naturalHeight})`
        )
      } else {
        console.log(
          `✓ Image ${i + 1} loaded successfully (dimensions: ${naturalWidth}x${naturalHeight})`
        )
      }
    }
    ...
    for (let i = 0; i < personalImageCount; i++) {
      ...
      console.log(
        `Personal Image ${i + 1}: Alt="${alt}", Src="${src}", Visible=${isVisible}`
      )
  ...
  test('Environment and Dependencies Check', async ({ page }) => {
    ...
    const errorElements = page.locator(
      'text*="Module not found", text*="Cannot resolve", text*="Failed to compile"'
    )
