...
test('Chat Response Detail Check', async ({ page }) => {
  ...
  const messageElements = page
    .locator(
      '[class*="space-y-2"] [role="paragraph"], [class*="space-y-2"] p, [class*="space-y-2"] div'
    )
    .filter({ hasText: /\w+/ })
  ...
  for (let i = 0; i < Math.min(messageCount, 10); i++) {
    ...
    const tagName = await messageElements
      .nth(i)
  ...
  const aiResponses = page.locator(
    'div:has(span:text("Swishter")) + div, p:has(span:text("Swishter")) + p'
  )
